...
export class SatisEmriOnayla extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      emptyField: true,
      ...
      value: undefined,
      data: {
        total_expected_sell_amount: undefined,
        orders_data: [],
  ...
  onClick(e) {
    if (e.target.id === "yatirim" && this.state.correct) {
      const url = "https://beta.miksinvest.com/emir/confirm-order";
      const formData = new FormData();
      const token = localStorage.getItem("jwtToken");
      const val=this.state.value;
      formData.append("invest_amount", this.state.value);
  ...
  handleChange(e) {
    ...
    if (val.toString().length > 4 && parseFloat(val) <= parseFloat(result)) {
      this.setState({ error: false, emptyField: false, correct:true,value:val });
      const url = "https://beta.miksinvest.com/miks/remove-fund";
      const formData = new FormData();
      ...
      this.setState({ error: false, emptyField: true,  correct: false });
    } else if (parseInt(val) > result) {
      this.setState({ error: true, errorMessage: "Yetersiz Bakiye",  correct: false });
    } else {
      console.log("unvalid input")
      this.setState({
        error: true,
        errorMessage: "Min 10.000 TL satabilirsiniz.",
        correct: false
      });
    }
  }
  ...
  componentWillMount() {
    const url =
      "https://beta.miksinvest.com/emir/satis-emri-onayla/" +
        ...
        <ul>
          ...
          <div className="ui segment">
            ...
            <div id="upper-section">
              <div id="left-section">
                <div>Satmak istediğiniz miktar</div>
                <hr />
              ...
              <div id="right-section">
                <div>
                  Tahmini Satılacak Miktar
                  <div id="rightAlign">
                    {this.state.emptyField
                      ? "--"
                      : this.state.fieldData.total_expected_sell_amount}
                  </div>
                </div>
                <hr />
                <div>
                ...
                <div>
                  Emir Tipi <div id="rightAlign">Piyasa Emri</div>
                </div>
              </div>
            </div>
