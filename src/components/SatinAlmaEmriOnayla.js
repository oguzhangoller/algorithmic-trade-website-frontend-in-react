...
export class SatinAlmaEmriOnayla extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      emptyField: true,
  ...
  onClick(e) {
    if (e.target.id === "yatirim"){ 
      ...
      formData.append("portfolio_id", this.props.match.params.id);
  ...
  handleChange(e) {
    ...
    if (val.toString().length > 4 && parseInt(val) <= result) {
      this.setState({ error: false, emptyField: false, value: val });
      const url = "https://beta.miksinvest.com/miks/add-fund";
      const formData = new FormData();
      ...
      this.setState({ error: false, emptyField: true });
    } else if (parseInt(val) > result) {
      this.setState({ error: true, errorMessage: "Yetersiz Bakiye" });
    } else {
      this.setState({
        error: true,
  ...
  componentWillMount() {
    const url =
      "https://beta.miksinvest.com/emir/satin-alma-emri-onayla/" +
                ...
                <div style={{ marginBottom: "10px" }}>
                  ...
                  <font style={{ float: "right", color: "black" }}>
                    {this.state.emptyField
                      ...
                      : this.state.fieldData.free_cash}
                  </font>
                </div>
                <div>
                  <Input
